...
- 舵机互动：表情点击、待机动作、小智表情反应、小智舵机命令和跳舞彩蛋共用同一套运动控制器与限速逻辑。表情和小智姿态保持水平 90 度、垂直 140 度的机械中位。
- 好感系统：Bond 页面显示分数、等级、心情和最近互动。好感分值现在会写入 NVS，重启后可恢复。
...
- 人脸检测：当前 Arduino 构建使用轻量级肤色启发式算法做粗略人脸区域估计与舵机居中，不是机器学习人脸检测，也不是人脸识别。
- 电源：电池电压现在通过 `M5.Power.getBatteryVoltage()` 读取。
...
- 菜单页：点击图标进入 Wi-Fi、Camera、Timer、Music、System 或 Servo Test。Back 返回表情页。
...
- Servo Test：进入后会以共享安全速度把舵机带回 90/90 测试中位。左右倾斜控制水平舵机，前后倾斜控制垂直舵机。点击回中，长按释放 PWM，Back 或左滑返回菜单并恢复表情 / 小智 90/140 中位。
- 休眠页：点击或摇晃可唤醒并回到表情页。
...
- 当前的人脸检测器只是基于肤色区域的启发式 blob 检测，可用于粗略居中，但不是严格意义上的人脸检测，更不是人脸识别。
